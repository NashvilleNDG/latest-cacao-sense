(function () {
  ...
  function initHeroSlider() {
    var slider = document.querySelector('[data-ts-hero-slider]');
    if (!slider) {
      return;
    }
    var slides = slider.querySelectorAll('.ts-hero__slide');
    if (slides.length < 2) {
      return;
    }
  ...
  function initHeroNext() {
    var btn = document.querySelector('[data-ts-reveal]');
    if (!btn) {
      return;
    }
    var targetId = btn.getAttribute('data-ts-reveal');
  ...
  function initPdpGallery() {
    ...
    var main = pdp.querySelector('[data-ts-pdp-main]');
    var thumbs = pdp.querySelectorAll('[data-ts-pdp-thumb]');
